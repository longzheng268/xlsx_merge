...
- 合并时保留首表表头、末表表尾
- 表体 H~AL（31 天考勤）区域内按考勤码自动填充对应色（H/HO/O/S/L/W/X）
...
- `--no-strict`：日志模式
- `--no-trim`：不剔除末尾空白行
- `--no-formula-adjust`：关闭 INDIRECT+ROW 动态绑定，改为相对行号静态平移
- `--no-col-width`：不保持列宽
...
- `adjust_formulas`：公式处理策略，`True`=INDIRECT+ROW() 动态绑定（默认），`False`=相对行号静态平移
- `copy_column_widths`：是否复制/计算列宽
- `col_width_a_g`：A-G 列固定列宽，默认 `7.24`
- `col_width_date`：日期列固定列宽（H 起，按实际天数），默认 `3.5`
...
4. 复制过程中保留：
   ...
   - 合并单元格
   - 行高
5. 表体会做逐行审核和语义校验，遇到明显的表头/表尾/说明行、只有序号无实质内容的行会跳过，并记录日志。此外两类模板无效信息也会被过滤：
   - B/C 列为模板占位符（形如 `XX`/`XXX`/`XXXXX` 的全 X 串）的行；
   - B~G（第 2~7 列）全空白、即使 H 起仍填有考勤码/公式的行。
6. 公式处理（两种策略，由 `adjust_formulas` 控制）：
   - `True`（默认，INDIRECT+ROW 动态绑定）：复制公式时，凡是涉及具体行号的相对引用（如 `H7`、`H7:AL7`）改写为 `INDIRECT("H"&ROW())` 之类动态绑定，使其无论落在输出表哪一行、或上方增删行，都自动引用当前所在行的对应列；
   - `False`（相对行号静态平移）：按「目标行-源行」偏移量直接平移行号——源第 7 行搬到输出第 7 行时行号不变，搬到第 20 行时 `H7` 变为 `H20`。
   绝对行引用（如 `$AA$1`）保持不动。可用 `--no-formula-adjust` 切换为静态平移。
7. 考勤码填色（默认开启）：表体考勤区域内，精确命中考勤码的单元格统一填充对应色（仅对表体数据行生效，表头/表尾原样搬运）。考勤天数按表头日号自动判断——28/29/30 天的月份只填充实际存在的天数列，不一定铺满到 AL：
